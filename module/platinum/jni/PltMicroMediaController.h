...
class PLT_OpenURITask : public PLT_ThreadTask
{
public:
    PLT_OpenURITask(PLT_MicroMediaController*   ctrl_point,
...
protected:
    PLT_MicroMediaController*   m_CtrlPoint;
...
public:
    ...
    void SetUUIDServer(NPT_String id);

    void SetUUIDRender(NPT_String id);
   ...
   // AVTransport
    ...
    bool        CanOpenURI() {return m_canOpenURI;}
    bool        CanPlayURI() {return m_canPlay;}
       
private:
    const char* ChooseIDFromTable(PLT_StringMap& table);
    PLT_MediaObject* ChooseItem();    
    ...
    void 	SetMediaServer(PLT_DeviceDataReference& device);
    void 	SetMediaRender(PLT_DeviceDataReference& device);
...
private:
    ...
    /* UUID for media server*/
    NPT_String m_UUIDServer;
    NPT_String m_UUIDRender;

    PLT_OpenURITask* m_openURITask;
};
