...
PLT_OpenURITask::PLT_OpenURITask(PLT_MicroMediaController*   ctrl_point, NPT_TimeInterval timer) :
...
PLT_OpenURITask::DoRun()
{
    ...
    while (!IsAborting((NPT_Timeout)m_Timer.ToSeconds()*1000)) {
        ...
        if (now < watchdog + NPT_TimeInterval(1.)) {
    	    continue;
        }
	...
	PLT_DeviceDataReference render;
	m_CtrlPoint->GetCurMediaRenderer(render);
	if (render.IsNull()){
	   NPT_LOG_INFO("render is null");
	   continue;
	}

	if (m_CtrlPoint->CanOpenURI()){
	   NPT_LOG_INFO("open URI");
	   m_CtrlPoint->openURI();
	}

        if (m_CtrlPoint->CanPlayURI()){
	   NPT_LOG_INFO("play URI");
	   m_CtrlPoint->HandleCmd_play();;
 	}

        //m_CtrlPoint->GetCurrentTransportActions(render, 0, NULL);
	NPT_LOG_INFO("get transport info");
	m_CtrlPoint->GetTransportInfo(render, 0, NULL);
    }
}
...
{     
    printf("=========%s,%s\n",
	device->GetUUID().GetChars(),
	device->GetFriendlyName().GetChars());

    SetMediaServer(device);
    ...
    if (m_openURITask == NULL) {
	NPT_LOG_INFO("start task :open uRI");
        // trigger 3 download
        NPT_TimeInterval delay(1.);

        m_openURITask = new PLT_OpenURITask(this, delay);

        //PLT_OpenURITask* task(new PLT_OpenURITask(this, delay));

        m_DownloadTaskManager.StartTask(m_openURITask, &delay);
    }
...
void
PLT_MicroMediaController:: SetUUIDServer(NPT_String id){
    ...
    if (id.GetLength()) {
        deviceList.Get(id, result);
    }

    if (result){
    	m_CurMediaServer = *result;	
    }else{
...
void 
PLT_MicroMediaController::SetUUIDRender(NPT_String id){

    m_UUIDRender = id;

    //m_MediaRenderers
    PLT_DeviceDataReference* result = NULL;
    NPT_AutoLock             lock(m_MediaServers);// why not 'm_MediaRenders'

    if (id.GetLength()) {
        m_MediaRenderers.Get(id, result);
    }
                                                    ...
                                                    NPT_List<PLT_StateVariable*>* vars)
{
    ...
    NPT_String name_play("CurrentTransportActions");
    NPT_String value_play("Play");

    NPT_String name_connectionId("CurrentConnectionIDs");

    bool bCanPlay = false;
    bool bCanOpenURI = false;
    while (var) {
	...
	bCanPlay = name_play.Compare((*var)->GetName(), true) ? false : true; 
	bCanPlay &= value_play.Compare((*var)->GetValue(),true) ? false : true;
       
 	bCanOpenURI = name_connectionId.Compare((*var)->GetName(), true) ? false : true; 
	++var;
    }
...
void 
PLT_MicroMediaController::OnGetCurrentTransportActionsResult(
        NPT_Result                res ,
...
{
    if (res != NPT_SUCCESS){
        printf("get current actions fail\n");
  	return;
    }

    NPT_List<NPT_String>::Iterator id = actions->GetFirstItem();    

    bool bCanOpenURI = false;    
    while (id){
	printf("%s ,",(*id).GetChars());
	NPT_String value_play("Play");

	bCanOpenURI = value_play.Compare((*id),true) ? false : true;
	if (bCanOpenURI){
	   //break;   
	}
	id++;
    ...
    //m_canOpenURI = bCanOpenURI;

    printf("\n");
}

void 
PLT_MicroMediaController::OnGetTransportInfoResult(
        NPT_Result                res ,
        PLT_DeviceDataReference&  device ,
...
{
    if (res != NPT_SUCCESS){
        printf("get transport info fail\n");
        return;
    }

    printf("state:%s, status:%s, speed:%s\n",
		info->cur_transport_state.GetChars(),
		info->cur_transport_status.GetChars(),
		info->cur_speed.GetChars());

    bool bCanPlay = false;
 
    NPT_String value_play("PLAYING");
    NPT_String value_stop("STOPPED"); 
    NPT_String value_noMedia("NO_MEDIA_PRESENT");
    bCanPlay = value_play.Compare(info->cur_transport_state, true) ? true : false;
    m_canOpenURI = value_stop.Compare(info->cur_transport_state, true) ? false : true;
    m_canOpenURI |=value_noMedia.Compare(info->cur_transport_state, true) ? false : true;

    m_canPlay = bCanPlay;

    printf("canOpenURI:%d,canPlay:%d\n",m_canOpenURI, bCanPlay);
    NPT_LOG_INFO_3("state:%s, status:%s, speed:%s\n",
                info->cur_transport_state.GetChars(),
                info->cur_transport_status.GetChars(),
                info->cur_speed.GetChars());
    NPT_LOG_INFO_2("canOpenURI:%d,canPlay:%d\n",m_canOpenURI, bCanPlay);
}
...
PLT_MicroMediaController::HandleCmd_getmr()
{
    ...
    if (!device.IsNull()) {
        printf("Current media renderer: %s,%s\n", 
		(const char*)device->GetFriendlyName());
    } else {
...
PLT_MediaObject* 
PLT_MicroMediaController::ChooseItem()
{
	NPT_List<NPT_String> idList;
	idList.Add("0");
	
        while (idList.GetItemCount()>0){
                NPT_List<NPT_String>::Iterator id = idList.GetFirstItem();
                NPT_String object_id = *id;
                idList.Erase(id);
		...
		DoBrowse(object_id);
                ...
                while (item) {
                        if ((*item)->IsContainer()) {
                                printf("%s,%d Container: %s (%s)\n",
					__FILE__,
					__LINE__, 
					(*item)->m_Title.GetChars(), 
					(*item)->m_ObjectID.GetChars());

                                idList.Add((*item)->m_ObjectID);
                        } else {
                                printf("Item: %s (%s)\n", 
					(*item)->m_Title.GetChars(), 
					(*item)->m_ObjectID.GetChars());
				...
				//NPT_LOG_INFO_2("Item: %s (%s)\n",
                                ...
                                PLT_MediaObject* track = *item;
                                if (track->m_Resources.GetItemCount() > 0) {
                                        NPT_Cardinal resource_index = 0;
            				
					NPT_String ctxType = track->m_Resources[resource_index].m_ProtocolInfo.GetContentType();
					printf("content type:%s\n",ctxType.GetChars());
					NPT_LOG_INFO_1("content type:%s\n",ctxType.GetChars());
            				if (ctxType.StartsWith("audio", true)){
						return track;
					}
                                }else{
                                        printf("number of resource is 0\n");
                                }
                        }
                        ++item;
                }
...
void        
PLT_MicroMediaController::openURI()
{
    printf("=================openURI\n");
    PLT_DeviceDataReference render;

    GetCurMediaRenderer(render);
    if (render.IsNull()){
	printf("render not ready\n");
	NPT_LOG_INFO("render not ready");
	return;
    }
    
    //Stop(render, 0, NULL);
    PLT_MediaObject* track = ChooseItem();
    if (track == NULL){
	    printf("can't find media object\n");
	    NPT_LOG_INFO("can't find media object");
	    return;
    }
    ...
    if (track->m_Resources.GetItemCount() > 0) {
	    ...
	    NPT_Cardinal resource_index = 0;
	    /*if (NPT_FAILED(FindBestResource(render, *track, resource_index))) {
	    ...
	    // invoke the setUri
	    NPT_LOG_INFO_1("set URI:%s",track->m_Title.GetChars());
	    SetAVTransportURI(render, 0, track->m_Resources[resource_index].m_Uri, track->m_Didl, NULL);
    } else {
...
void
PLT_MicroMediaController::SetMediaServer(PLT_DeviceDataReference& device)
{
    NPT_AutoLock lock(m_CurMediaServerLock);
   
    //bool bMatched = m_UUIDServer.Compare(device->GetUUID(), true) ? false : true;
    bool bMatched = device->GetUUID().EndsWith("dms", true);
    if (!bMatched){
	printf("UUID not match:%s,%s\n",
			device->GetUUID().GetChars(),
			m_UUIDServer.GetChars());
	NPT_LOG_INFO_2("UUID not match:%s,%s\n",
                        device->GetUUID().GetChars(),
                        m_UUIDServer.GetChars());
    }else{
	m_CurMediaServer = device;
	printf("set media server:%s\n",m_CurMediaServer->GetUUID().GetChars());
	NPT_LOG_INFO_1("set media server:%s\n",m_CurMediaServer->GetUUID().GetChars());
...
void
PLT_MicroMediaController::SetMediaRender(PLT_DeviceDataReference& device)
{
    NPT_AutoLock lock(m_CurMediaRendererLock);
    bool bMatched = device->GetUUID().EndsWith("dmr", true);
    if (!bMatched){
        printf("UUID not match:%s,%s\n",
                        device->GetUUID().GetChars(),
                        m_UUIDRender.GetChars());
    }else{
        m_CurMediaRenderer = device;
        printf("set media render:%s\n",m_CurMediaRenderer->GetUUID().GetChars());
    }
}
            ...
            if (object_id.GetLength()) {
                ...
                if (NPT_SUCCEEDED(NPT_ContainerFind(*m_MostRecentBrowseResults, PLT_MediaItemIDFinder(object_id), track))) {
                    if (track->m_Resources.GetItemCount() > 0) {
                        ...
                        // invoke the setUri
                        printf("Issuing SetAVTransportURI with =%s & didl=%s", 
                            (const char*)track->m_Resources[resource_index].m_ProtocolInfo.GetProtocol(), 
                            (const char*)track->m_Didl);
...
PLT_MicroMediaController::HandleCmd_help()
{
    ...
    printf(" uuid_r  -   set uuid for current media renderer\n");
    printf(" uuid_s  -   set uuid for current media server\n");
...
PLT_MicroMediaController::ProcessCommandLoop()
{
    ...
    while (!abort) {
        ...
        if (0 == strcmp(command, "quit") || 0 == strcmp(command, "exit")) {
            ...
            HandleCmd_mute();
	} else if (0 == strcmp(command, "uuid_s")) {
            char id[1024];
	    ...
	    SetUUIDServer(id);
	} else if (0 == strcmp(command, "uuid_r")) {
            char id[1024];
